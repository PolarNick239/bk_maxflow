...
template <typename captype, typename tcaptype, typename flowtype>
	typename Graph<captype, tcaptype, flowtype>::termtype Graph<captype, tcaptype, flowtype>::what_segment(node_id _i)
{
	if (nodes[_i].parent && !(nodes[_i].is_sink)) return SOURCE;
	return SINK;
}
